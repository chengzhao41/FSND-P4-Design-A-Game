...
from models import StringMessage, NewGameForm, GameForm, MakeMoveForm,\
    ScoreForms, EmailPreferenceForm
...
class TicTacToeApi(remote.Service):
    ...
    def _check_for_win(board_position, player_piece):
        """Check if the player with the player_piece won the game."""
        # check all the rows
        ...
        for row in range(0, 3):
            ...
            for col in range(0, 3):
                ind = row * 3 + col
                print board_position[ind]
                if board_position[ind] != player_piece:
                    win = False
                    break
            print "win + " + str(win)
            if win:
        ...
        print "checking the forward diagonal"
        win = True
        for col_and_row in range(0, 3):
            ind = col_and_row * 3 + col_and_row
        ...
        print "checking the backward diagonal"
        win = True
        for col_and_row in range(0, 3):
            ind = col_and_row * 3 + (2 - col_and_row)
            if board_position[ind] != player_piece:
                win = False
                break
